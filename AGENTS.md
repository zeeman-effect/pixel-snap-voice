...
- **JLC SMT assembly is sourced.** Every part on `jlcpcb_bom.csv` / `jlcpcb_cpl.csv` has an LCSC code in `docs/bom.md`. `generate_jlc.py` refuses to write those CSVs if one is missing. **J1** (UART header) and **MK1** (IM73A135) are flagged `exclude_from_bom` and `exclude_from_pos_files`; solder those by hand. **BT1** stays on the BOM for wave solder and stays out of the pick-and-place file.
