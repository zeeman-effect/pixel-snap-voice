...
5. Upload `hardware/kicad/fab/` Gerbers + `hardware/kicad/jlcpcb_bom.csv` + `jlcpcb_cpl.csv` to JLCPCB. Step 2 writes all three from `recorder.kicad_pcb`, so upload the files that run just produced. Do not hand-edit the CSVs: LCSC order codes come from the table in [`docs/bom.md`](bom.md), and which parts the machine places comes from the board's own `exclude_from_pos_files` / `exclude_from_bom` flags. A missing LCSC on a machine-placed part now fails the gate instead of shipping a blank feeder. **J1** and **MK1** are off both CSVs; solder those by hand after the board comes back. **BT1** is on the BOM for wave solder, not in the pick-and-place file. **SP1:** JLC places C18186315 from their EasyEDA library 0° (`BUZ-SMD_4P-L13.0-W13.0-P11.4-BL`, pin 1 bottom-left), not this KiCad land's 0° (pad 1 bottom-right). CPL rotation is 0. In the assembly preview, the coil must sit on the numbered pads toward the analog island, not on the dummy pair toward the top edge. A left-right swap only inverts polarity. 180° leaves the speaker open.
